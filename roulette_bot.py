...
# Image fiable (remplace par ta photo uploadée sur Imgur si tu veux)
...
def main_keyboard():
    ...
    markup.add(InlineKeyboardButton("💰 Solde", callback_data="balance"),
...
def callback_handler(call):
    ...
    if call.data == "bet_menu":
        try:
            ...
            bot.send_message(chat_id, "🪙 **Table PATOUCH** - Placez vos jetons", reply_markup=bet_keyboard())
    ...
    elif call.data.startswith("bet_") or call.data.startswith("num_"):
...
def show_number_buttons(chat_id):
    ...
    bot.send_message(chat_id, "🔢 Choisissez un numéro (mise 100) :", reply_markup=markup)
