import telebot
import random
import time
import json
import os
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton

TOKEN = os.getenv("TOKEN")
ADMIN_ID = int(os.getenv("ADMIN_ID", 0))

bot = telebot.TeleBot(TOKEN)

DATA_FILE = "casino_data.json"
RIGGED_MODE = True

# Image de table de roulette (tu peux changer l'URL)
TABLE_IMAGE = "https://i.imgur.com/0Z9vK8J.jpg"   # Table de roulette européenne classique

def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"users": {}, "current_round": {"bets": [], "active": False}}

def save_data():
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

data = load_data()

def get_user(user_id):
    uid = str(user_id)
    if uid not in data["users"]:
        data["users"][uid] = {"balance": 1000, "history": []}
    return data["users"][uid]

def get_color(n):
    if n == 0: return "🟢 Vert"
    reds = [1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36]
    return "🔴 Rouge" if n in reds else "⚫ Noir"

# ====================== CLAVIERS ======================
def main_keyboard():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(InlineKeyboardButton("🪙 Placer Jetons", callback_data="bet_menu"),
               InlineKeyboardButton("🎡 Tourner la Roue", callback_data="spin_now"))
    markup.add(InlineKeyboardButton("🏆 Classement", callback_data="leaderboard"),
               InlineKeyboardButton("💰 Solde", callback_data="balance"))
    return markup

def bet_keyboard():
    markup = InlineKeyboardMarkup(row_width=3)
    amounts = [50, 100, 250, 500]
    simple = ["rouge", "noir", "pair", "impair", "manque", "passe"]
    for t in simple:
        for a in amounts:
            markup.add(InlineKeyboardButton(f"{t.capitalize()} {a}", callback_data=f"bet_{t}_{a}"))
    
    markup.row(
        InlineKeyboardButton("Colonne 1", callback_data="bet_col1_50"),
        InlineKeyboardButton("Colonne 2", callback_data="bet_col2_50"),
        InlineKeyboardButton("Colonne 3", callback_data="bet_col3_50")
    )
    markup.row(
        InlineKeyboardButton("Douzaine 1", callback_data="bet_douz1_50"),
        InlineKeyboardButton("Douzaine 2", callback_data="bet_douz2_50"),
        InlineKeyboardButton("Douzaine 3", callback_data="bet_douz3_50")
    )
    markup.add(InlineKeyboardButton("🔢 Choisir un Numéro", callback_data="choose_number"))
    return markup

# ====================== MISES ======================
@bot.callback_query_handler(func=lambda call: True)
def callback_handler(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    user = get_user(user_id)

    if call.data == "bet_menu":
        bot.send_photo(chat_id, TABLE_IMAGE, caption="🪙 **Table de Roulette** - Placez vos jetons", reply_markup=bet_keyboard())
    elif call.data == "choose_number":
        bot.send_message(chat_id, "🔢 **Entrez votre mise sur un numéro**\n\nExemple : `17 100`")
    elif call.data.startswith("bet_"):
        handle_bet(call, user, chat_id)
    elif call.data == "spin_now":
        do_spin(chat_id)
    elif call.data == "balance":
        bot.send_message(chat_id, f"💰 **Solde actuel** : {user['balance']} crédits")
    elif call.data == "leaderboard":
        show_leaderboard(chat_id)

def handle_bet(call, user, chat_id):
    try:
        parts = call.data.split("_")
        btype = parts[1]
        amount = int(parts[2])

        if user["balance"] < amount:
            bot.answer_callback_query(call.id, "❌ Solde insuffisant", show_alert=True)
            return

        user["balance"] -= amount

        if not data["current_round"]["active"]:
            data["current_round"]["active"] = True
            data["current_round"]["bets"] = []

        data["current_round"]["bets"].append({
            "user_id": call.from_user.id,
            "username": call.from_user.first_name,
            "type": btype,
            "amount": amount
        })

        save_data()
        bot.answer_callback_query(call.id, f"✅ {amount} sur {btype}")

        # Affichage des mises en cours
        show_current_bets(chat_id)

    except:
        bot.answer_callback_query(call.id, "Erreur")

def show_current_bets(chat_id):
    bets = data["current_round"].get("bets", [])
    if not bets:
        return
    text = "📋 **Mises en cours :**\n"
    for b in bets:
        text += f"• {b['username']} : {b['amount']} sur **{b['type']}**\n"
    bot.send_message(chat_id, text)

# ====================== SPIN ======================
def do_spin(chat_id):
    bets = data["current_round"].get("bets", [])
    if not bets:
        bot.send_message(chat_id, "❌ Aucune mise.")
        return

    bot.send_animation(chat_id, "https://media.giphy.com/media/3o7aDgf7a9Q5cJ4qM0/giphy.gif", caption="🎡 **La boule tourne...**")

    time.sleep(4)

    result = random.randint(0, 36)
    color = get_color(result)
    text = f"🎰 **Résultat : {result} {color}**\n\n"

    for bet in bets:
        win = calculate_win(bet["type"], result, bet["amount"])
        user = get_user(bet["user_id"])
        if win > 0:
            user["balance"] += win
            text += f"✅ {bet['username']} gagne **{win}**\n"
        else:
            text += f"❌ {bet['username']} perd sa mise\n"

    data["current_round"] = {"bets": [], "active": False}
    save_data()
    bot.send_message(chat_id, text + "\n🎲 Table prête pour le prochain tour.", reply_markup=main_keyboard())

def calculate_win(btype, result, amount):
    if btype in ["rouge", "noir"] and btype in get_color(result).lower():
        return amount * 2
    if btype in ["pair", "impair", "manque", "passe"] and check_simple(btype, result):
        return amount * 2
    if btype.startswith(("col","douz")) and check_col_douz(btype, result):
        return amount * 3
    if btype.isdigit() and int(btype) == result:
        return amount * 36
    return 0

def check_simple(t, n):
    if t == "pair": return n % 2 == 0 and n != 0
    if t == "impair": return n % 2 == 1
    if t == "manque": return 1 <= n <= 18
    if t == "passe": return 19 <= n <= 36
    return False

def check_col_douz(t, n):
    if t in ["col1","douz1"]: return 1 <= n <= 12
    if t in ["col2","douz2"]: return 13 <= n <= 24
    if t in ["col3","douz3"]: return 25 <= n <= 36
    return False

# Commande pour numéro précis
@bot.message_handler(commands=['bet_numéro'])
def bet_number(message):
    try:
        _, num, amt = message.text.split()
        num, amt = int(num), int(amt)
        user = get_user(message.from_user.id)
        if amt > user["balance"] or not 0 <= num <= 36:
            bot.reply_to(message, "❌ Valeur invalide")
            return
        user["balance"] -= amt
        if not data["current_round"]["active"]:
            data["current_round"]["active"] = True
            data["current_round"]["bets"] = []
        data["current_round"]["bets"].append({
            "user_id": message.from_user.id,
            "username": message.from_user.first_name,
            "type": str(num),
            "amount": amt
        })
        save_data()
        bot.reply_to(message, f"✅ Mise {amt} sur le numéro **{num}**")
        show_current_bets(message.chat.id)
    except:
        bot.reply_to(message, "Usage : `/bet_numéro 17 100`")

@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(message.chat.id, "🌟 **Casino Royale** - Table de Roulette ouverte !", reply_markup=main_keyboard())

print("🚀 Bot avec Table Visuelle Prêt !")
bot.infinity_polling()
